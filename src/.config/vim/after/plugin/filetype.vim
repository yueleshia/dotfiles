...
noremap <unique> <silent> <Leader>q :execute('silent ![ -n "${TMUX}" ] && tmux new-window -d build.sh background --temp ' . shellescape(expand('%')))<CR>:redraw<CR>
noremap <unique> <silent> <Leader>w :call system('tmux-alt-pane.sh send-keys -X cancel; tmux-alt-pane.sh send-keys "build.sh build --temp "' . shellescape(expand('%')) . " Enter")<CR>
noremap <unique> <silent> <Leader>e :call system('tmux-alt-pane.sh send-keys -X cancel; tmux-editor-run.sh ' . expand('%:e') . ' ' . shellescape(expand('%:p')))<CR>
noremap <unique> <silent> <Leader>r :call system('tmux-alt-pane.sh send-keys -X cancel; tmux-alt-pane.sh send-keys "build.sh run --temp "' . shellescape(expand('%')) . " Enter")<CR>
noremap <unique> <silent> <Leader>t :call system('tmux-alt-pane.sh kill-pane')<CR>
noremap <unique> <silent> <Leader>l :call system('tmux-alt-pane.sh send-keys -X cancel; tmux-alt-pane.sh send-keys "build.sh lint "' . shellescape(expand('%')) . " Enter")<CR>
